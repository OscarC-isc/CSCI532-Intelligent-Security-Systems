...
def main(grid: Grid, context: Context) -> None:
    ...
    pid_enabled = context.run_config.get("pid-enabled", False)
    ...
    if pid_enabled:
        ...
        print(f"  Threshold: {pid_threshold}")
    print("\n")
    ...
    exclusion_summary = strategy.get_exclusion_summary()
    print("\n" + "=" * 70)
    print("FINAL PID EXCLUSION SUMMARY")
    print("=" * 70)
    print(f"Total Clients Excluded: {exclusion_summary['total_excluded']}")
    print(f"Excluded Clients: {exclusion_summary['excluded_clients']}")
    print(f"")
    print(f"Detection Performance:")
    print(f"  True Positives (Correctly Excluded Poisoned):  {exclusion_summary['true_positives']}")
    print(f"  False Positives (Wrongly Excluded Clean):     {exclusion_summary['false_positives']}")
    print(f"  False Negatives (Missed Poisoned Clients):    {exclusion_summary['false_negatives']}")
    print(f"")

    if len(poisoned_clients) > 0:
        precision = exclusion_summary['true_positives'] / max(1, exclusion_summary['total_excluded'])
        recall = exclusion_summary['true_positives'] / max(1, len(poisoned_clients))
        f1 = 2 * (precision * recall) / max(0.001, precision + recall)

        print(f"Detection Metrics:")
        print(f"  Precision: {precision:.2%} (of excluded, how many were actually poisoned)")
        print(f"  Recall:    {recall:.2%} (of poisoned, how many were detected)")
